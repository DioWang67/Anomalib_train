...
class DataAugmentor:
    ...
    def _setup_output_dirs(self):
        """建立必要的輸出目錄"""
        try:
            output_dir = Path(self.config['output']['image_dir'])
            output_dir.mkdir(parents=True, exist_ok=True)
            self.logger.info(f"創建輸出目錄成功: {output_dir}")
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config_img_annotator.yaml', help='配置文件路徑')
...
if __name__ == "__main__":
    main()
    # python anomalib_img_Auto_Annotator.py --config config_img_annotator.yaml